    ...
    // Thêm nhiều sinh viên vào lớp (dựa trên danh sách sinh viên)
    router.post("/classes/:Ma_Lop/add-multiple-students", authenticateAdmin, async (req, res) => {
        try {
            const { Ma_Lop } = req.params;
            ...
            const [classInfo] = await db.query("SELECT So_Luong FROM LOP WHERE Ma_Lop = ?", [Ma_Lop]);
            if (classInfo.length === 0) {
                return res.status(404).json({ success: false, message: "Lớp học không tồn tại." });
            }

            const maxQuantity = classInfo[0].So_Luong;
            const [currentStudents] = await db.query("SELECT COUNT(*) AS count FROM SINHVIEN WHERE Ma_Lop = ?", [Ma_Lop]);
            let availableSlots = maxQuantity - currentStudents[0].count;
    ...
    for (const studentIdRaw of students) {
        ...
        if (!studentId || availableSlots <= 0) continue;

        // Kiểm tra sinh viên trong USER
        const [user] = await db.query("SELECT Vai_Tro FROM USER WHERE Tai_Khoan = ?", [studentId]);
        if (user.length === 0 || user[0].Vai_Tro !== 'SinhVien') {
            skippedStudents.push({ studentId, reason: "Không tồn tại hoặc không phải là sinh viên" });
            continue;
        }

        // Kiểm tra đang có lớp khác
        const [currentClass] = await db.query(
            "SELECT Ma_Lop FROM SINHVIEN WHERE Ma_Sinh_Vien = ? AND Ma_Lop IS NOT NULL",
            [studentId]
        );
        if (currentClass.length > 0 && currentClass[0].Ma_Lop !== Ma_Lop) {
    skippedStudents.push({ studentId, reason: `Đang thuộc lớp ${currentClass[0].Ma_Lop}` });
            continue;
        }

       await db.query("UPDATE SINHVIEN SET Ma_Lop = ? WHERE Ma_Sinh_Vien = ?", [Ma_Lop, studentId]);
        ...
        availableSlots--;
            ...
            console.error("Lỗi thêm nhiều sinh viên:", err);
