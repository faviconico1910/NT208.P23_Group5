...
def generate_ma_sv(index):
    return f"SV{index:04d}"  # Format SVxxxx, ví dụ: SV0001, SV0821

def generate_unique_tai_khoan(existing_ids):
    while True:
        nam_nhap_hoc = random.randint(18, 25)  # Hai số đầu là năm (18 -> 25)
        ma_nganh_mac_dinh = "52"  # Hai số tiếp theo mặc định là 52
        so_ngau_nhien = f"{random.randint(1000, 9999)}"  # Bốn số cuối ngẫu nhiên
        ma_sv = f"{nam_nhap_hoc}{ma_nganh_mac_dinh}{so_ngau_nhien}"
        
        if ma_sv not in existing_ids:
            existing_ids.add(ma_sv)  # Thêm vào danh sách đã tồn tại
            return ma_sv
...
def main():
    ...
    ds_he_dao_tao = ["Chính quy", "Chất lượng cao", "Liên thông"]
    ...
    with open("insert_sinhvien.sql", "w", encoding="utf-8") as file:
        for i in range(1, 1001):
            ma_sv = generate_ma_sv(i) 
            ho_ten = faker.name()
            gioi_tinh = random.choice(["Nam", "Nữ"])
            ngay_sinh = faker.date_of_birth(minimum_age=18, maximum_age=25).strftime("%Y-%m-%d")
            ...
            ma_lop = f"{ma_nganh}{nam}"
            tai_khoan = generate_unique_tai_khoan(existing_ids)
            he_dao_tao = random.choice(ds_he_dao_tao)
            email_truong = f"{tai_khoan}@gm.uit.edu.vn"
